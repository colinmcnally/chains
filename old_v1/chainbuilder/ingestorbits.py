...
runtable[:]['key'] = [ bytes.fromhex(key) for key in chainmodels.runs.keys()]

runtuples = np.asarray(list(chainmodels.runs.values()))
...
for key in chainmodels.runs:
  print(key,' ',chainmodels.runs[key])
  orb = h5py.File(chainmodels.filenames[key], 'r')
