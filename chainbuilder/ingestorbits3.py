...
libf = h5py.File('chain_data_library3.h5', 'w')

#sha1 gives 40 byte hash. so this datatype relies on that choice as the run encoding
runtable = np.zeros(len(chainmodels.runs_ext2), 
...
runtable[:]['key'] = [ bytes.fromhex(key) for key in chainmodels.runs_ext2.keys()]

runtuples = np.asarray(list(chainmodels.runs_ext2.values()))
