...
plt.ylabel('Longitude of Periastron Differences')

lkol = lko[:][:,-1]
anominal = lkol['a'][0] * np.ones(nchain)
for i in range(1,nchain):
    anominal[i] = anominal[i-1] * (5.0/4.0)**(2.0/3.0)
aactual = lkol['a'][:]
e = lkol['e'][:]

mu = np.zeros(nchain)
beta = np.zeros(nchain)
Delta0 = np.zeros(nchain)
Delta = np.zeros(nchain)
G = np.zeros(nchain)
for i in range(0,nchain):
    #assume G is one, 
    mu[i] = (sa[0].particles[0].m + sa[0].particles[i+1].m)
    beta[i] = sa[0].particles[0].m * sa[0].particles[i+1].m / (sa[0].particles[0].m + sa[0].particles[i+1].m)
    Delta0[i] = beta[i] * np.sqrt(mu[i] * anominal[i])
    Delta[i] = beta[i] * np.sqrt(mu[i] * aactual[i])
    G[i] = Delta[i] *np.sqrt(1.0 -e[i]**2)

epsilon = (Delta[i] -Delta0[i]).sum()
...
print('epsilon/Delta0.sum() ',(Delta/Delta0).sum()-nchain)
print('epsilon/Delta0.sum() ',epsilon/Delta0.sum())

print(oa.compute_tight_angles(lko))
